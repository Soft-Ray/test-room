...
당신이 또 이 방을 나가면 완전히 탈출한 거라 볼 수 있을까요? 
빨리 답을 찾기를. 행운을 빕니다.
ps.'맞아'라는 긍정으로 대답하는 것은 진실로 이끌어낼지도 몰라요.
-관리자-`
          },
          {
            type: 'steve',
    ...
    if (userMessage.startsWith('check_')) {
      ...
      if (action === 'bookshelf') {
        ...
        if (!frameChecked) {
          ...
          return res.json({
            message: `${effectiveName}님, 책장을 보니 위에는 2개를 꽂을 수 있고 중간에는 1개 맨 밑에는 2개를 꽂을 수 있어요. 바닥에 흰색, 노란색, 초록색, 빨간색, 파란색 책이 떨어져 있네요!`,
            image: 'Save-steve.gif'
          });
        }
      }
 ...
 // 액자 관련 처리
    if (userMessage.includes('그림')) {
      ...
      if (pictureMoved) {
        ...
        return res.json({
          message: `${effectiveName}님, 액자에는 아름다운 풍경화가 그려져 있어요. 산과 강이 있는 평화로운 그림이네요. 그런데 액자가 조금 기울어져 있는 것 같아요... '내려줘'라고 말해보세요.`,
          image: 'images/sup.gif'
        });
      }
    }
...
시간의 흐름은 반복을 품었으며,
끝은 언제나 선택을 요구했다.
*버튼을 4개 누를 수 있습니다, '버튼을 누르고 싶어.'라고 입력해보세요!`
            },
            {
              type: 'steve',
              text: `이제 버튼 4개를 눌러볼 수 있을 것 같아요, 버튼을 눌러볼까요?`
            }
          ],
          image: 'images/hap.gif'
        });
...
📚 책장 정보:
위에는 2개, 중간에는 1개, 맨 밑에는 2개를 꽂을 수 있어요.
바닥에는 흰색, 노란색, 초록색, 빨간색, 파란색 책이 떨어져 있어요!

🎯 다음 단계:
맨 왼쪽 위부터 책을 꽂기 시작합니다. 
'00색,00색,00색,00색,00색 책을 꽂는다'라고 말해보세요!`,
            image: 'images/hap.gif',
            forceNote: true
          });
        } else {
...
진실은 가장 밝은 색이어야 하니, 중심에 그것이 있어야 한다…`
          ...
          {
            type: 'steve',
            text: `${effectiveName}님, 무슨 말일까요...? 어렵네요. 책장을 보니 위에는 2개를 꽂을 수 있고 중간에는 1개 맨 밑에는 2개를 꽂을 수 있어요... 바닥에는 책이 떨어져있어요 흰색, 노란색, 초록색, 빨간색, 파란색 책이에요...!`
          },
          {
            type: 'narration',
            text: `맨 왼쪽 위부터 책을 꽂기 시작합니다. 순서를 잘 생각해보세요. '00색,00색,00색,00색,00색 책을 꽂는다.' 라고 스티브에게 말해보세요.`
          }
        ],
        image: 'images/sup.gif'
      });
    ...
    if (userMessage.includes('책장') || userMessage.includes('책들') || 
      ...
      if (safeOpened && bookshelfChecked) {
        return res.json({
          message: `${effectiveName}님, 책장을 보니 위에는 2개, 중간에는 1개, 맨 밑에는 2개를 꽂을 수 있어요. 바닥에 흰색, 노란색, 초록색, 빨간색, 파란색 책이 떨어져 있네요!`,
          image: 'Save-steve.gif'
        });
