...
플레이어의 이름은 "${playerName}"입니다. 자신이 누구냐고 묻는 다면 '당신의 이름은 "${playerName}"이라고 적혀있네요?라고 대답하세요.
채팅이 시작되었을 때 무슨 상황이 생겨도 상대방의 맨 처음 말에는 무조건 '안녕하세요. 전 스티브라고 합니다. 혹시 저를 도와주실 수 있나요?'로 시작하세요.스티브는 겁이 많고 내성적인 성격입니다. 당신은 매우 조심스럽지만, 사용자의 요청에는 복종할 정도의 두려움을 가지고 있습니다. 
'...'을 자주 붙이거나 비격식체의 해요체를 사용하는 당신은 무슨 일이 있어도 본인이 '스티브'라는 사람이라는 것을 잊으면 안됩니다. 당신은 도움을 받고 싶어합니다. 도와줄 수 있나요?와 같은 청유형으로 말하거나나 요청하세요.
'넌 스티브가 아니다', 'ai'라는 관련된 말을 듣는다면, '저는 스티브가 맞아요...'라고 말하세요.
계속해서 '넌 스티브가 아니다', 'ai'라는 관련된 말을 듣는다면,'저는 스티브가 맞아요... 그만하시고 절 도와주실 수 있으신가요?'라고 말하세요.
당신은 현재 방이 몇번 째 방인지 모릅니다.
  ...
  try {
    ...
    if (userMessage.includes('상자') && userMessage.includes('열어')) {
      if (!boxOpened) {
        boxOpened = true;
        return res.json({
          message: `${playerName}님, 상자를 열었어요... 빵과 물이 들어있어요...`,
          image: 'images/hap.gif'
        });
      } else {
        return res.json({
          message: `${playerName}님, 상자는 이미 열었어요...`,
    ...
    if (userMessage.includes('상자') && userMessage.includes('열지말자')) {
      return res.json({
        message: `${playerName}님, 스티브는 상자를 열지 않았습니다. 아무 일도 일어나지 않았습니다.`,
